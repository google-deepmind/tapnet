...
def plot_tracks_v2(
  ...
  Args:
    rgb: frames of shape [num_frames, height, width, 3].  Each frame is
      passed directly to plt.imshow.
    points: tracks, of shape [num_points, num_frames, 2], np.float32.
      [0, width / height]
    ...
    trackgroup: Optional, shape [num_points], int: grouping labels for the
      plotted points.  Points with the same integer label will be plotted
      with the same color.  Useful for clustering applications.
    point_size: int, the size of the plotted points, passed as the 's'
      parameter to matplotlib.
