...
flags.DEFINE_string(
    'input_pkl_path', None, 'Path to the pickle file.', required=True)
flags.DEFINE_string(
...
def main(argv: Sequence[str]) -> None:
  ...
  logging.info('Loading data from %s. This takes time.', FLAGS.input_pkl_path)
  with open(FLAGS.input_pkl_path, 'rb') as f:
    data = pickle.load(f)
  ...
  frames = data[idx]['video']
  point_tracks = data[idx]['points'].astype(np.float32)
  occluded = data[idx]['occluded'].astype(bool)
