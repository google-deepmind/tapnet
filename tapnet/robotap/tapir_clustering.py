...
def compute_clusters(
  ...
  separation_visibility_dict = jax.tree_util.tree_map(
...
def construct_fake_causal_state(
  ...
  if convert_to_jax:
    fake_ret = jax.tree_util.tree_map(jnp.array, fake_ret)
  return fake_ret
...
def track_many_points(
  ...
  for qf_idx, query_features in enumerate(query_features2):
    ...
    for sv_idx, sv in enumerate(
      ...
      for i in range(len(sv)):
        ...
        prediction = jax.tree_util.tree_map(np.array, prediction)
  ...
  return {
      ...
      'query_features': jax.tree_util.tree_map(np.array, out_query_features),
      'demo_episode_ids': demo_episode_ids,
