...
"""Visualizes homography augmentation for TAP-Next long context training."""

import argparse
import cv2
import imageio
import numpy as np
from .homography import HomographyAugmentation
...
def visualize_homography(output_path):
  """Generates and saves frames demonstrating homography augmentation."""
  gif_frames = []
  h, w = 256, 256
  s = 128  # num frames

  video = [create_dummy_frame(i, s, height=h, width=w) for i in range(s)]
  video = np.stack(video)

  data = {
      'rgb/encoded': video,
      'trajs_2d': np.zeros((s, 0, 2)),  # no trajectories needed for viz
  }

  aug = HomographyAugmentation(p=1.1, strength=1.0)  # force augmentation
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--output_path',
      default='/tmp/homography_viz.gif',
      help='Output path for visualization GIF.',
  )
  args = parser.parse_args()
  visualize_homography(args.output_path)
