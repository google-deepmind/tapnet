...
class TAPNextPP(nn.Module):
  ...
  def from_checkpoint(
    ...
    Args:
        ...
        input_resolution: Pixel resolution frames are resized to before patch
          embedding. Use ``256`` for standard checkpoints or ``512`` for
          checkpoints fine-tuned at higher resolution.
    ...
    inner = tapnext_torch.TAPNext(image_size=(cls.MODEL_SIZE, cls.MODEL_SIZE))
  ...
  def track_frame(
      ...
      state: tapnext_torch.TAPNextTrackingState | None = None,
      autocast: bool = True,
  ) -> Tuple[np.ndarray, np.ndarray, tapnext_torch.TAPNextTrackingState]:
    """Process one frame and return tracking results in display coordinates.

    Args:
        frame_bgr: [H, W, 3] uint8 BGR frame (OpenCV layout).
        query_points_xy: [Q, 2] float32 array of [x, y] coordinates in display
          pixels. Must be provided when ``state`` is ``None``.
    ...
    frame_t = utils.preprocess_frame(
        frame_bgr, self.device, self.input_resolution
    )

    q_t: torch.Tensor | None = None
    if query_points_xy is not None:
      model_pts = utils.display_to_model(query_points_xy, h, w, self.MODEL_SIZE)
      q_t = utils.make_query_tensor(model_pts, self.device)
    ...
    positions_xy = utils.model_to_display(tracks_xy, h, w, self.MODEL_SIZE)
  ...
  def warmup(self, n_points: int = 64) -> None:
    ...
    with torch.amp.autocast("cuda", dtype=torch.float16):
      _, _, _, state = self._model(
          video=dummy_video, query_points=dummy_queries
      )
      for _ in range(3):
