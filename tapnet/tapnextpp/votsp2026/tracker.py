...
def _get_checkpoint() -> str:
    ...
    cache_dir = Path.home() / ".cache" / "tapnextpp"
...
def main() -> None:
    ...
    if n_support > 0:
        print(
            f"Using {n_support} support points ({support_mode} mode) per offset group "
            f"(not saved to output)."
        )
    ...
    for offset, group in sorted(by_offset.items()):
        ...
        def get_first_frame():
            ...
            if first_frame_img is None:
                first_frame_img = cv2.imread(frame_paths[offset])
