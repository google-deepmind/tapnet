...
**VOTSp2026 result:** d_avg = 0.647 (δ@1px=0.274, δ@2px=0.491, δ@4px=0.721, δ@8px=0.842, δ@16px=0.908), n=50 sequences.
...
```
It is cached to `~/.cache/tapnextpp/tapnextpp_512.ckpt` and reused on subsequent runs.
...
The resulting zip must have `identifier: TNPP` in its `manifest.yml` to match the challenge registration.
...
The VOT toolkit runs the tracker subprocess once per sequence. For each run it writes a temporary directory containing:
